...
<!--
  Two-row layout:
    Row 1: OSC 1 | OSC 2 | FILTER
    Row 2: ENVELOPE (wide, 2 cols) | LFO
-->
<div class="grid h-full min-h-0 grid-cols-3 grid-rows-[minmax(0,1fr)_minmax(0,1.4fr)] gap-3">
	<!-- Row 1 -->
	<div class="min-h-0">
		<OscCard which="osc1" />
	</div>
	...
	<!-- Row 2 -->
	<section
		class="col-span-2 flex min-h-0 flex-col rounded-lg border border-surface0 bg-mantle/60 p-3"
	>
		<Envelope />
	</section>
	<section class="flex min-h-0 flex-col gap-2 rounded-lg border border-surface0 bg-mantle/60 p-3">
