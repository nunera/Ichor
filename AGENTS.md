...
- **Per-route amount targets** are auto-managed via `#syncRouteAmtTargets()` whenever routes mutate or a patch loads. Id format: `source.route.{targetId}.amount` (where source is `lfo1`, `lfo2`, `modEnv1`, or `modEnv2`). The mod-tick reads `liveMod.get(...)` for those ids with one frame of lag. Note that the patch `amount` is bipolar (-0.5 to 0.5) while the live mod target `getBase` returns the absolute magnitude (0 to 0.5), with the engine applying the sign during accumulation.
- **Drag blockers:** Drawers and overlays register themselves as drag blockers in `dragMod.svelte.ts` so users cannot route "through" open UI panels.
...
- **Mod envelopes** (`modEnv1`/`modEnv2`) are `Tone.Envelope` instances (not connected to any audio node). Their value is sampled each `#modTick` via `getValueAtTime()` (0..1 output) and accumulated like LFOs. `Tone.Envelope` has no native `hold` phase, so `#applyModEnvOne()` emulates it by folding `hold` into the attack time and providing a custom `attackCurve` array. Hold amount is per-route, bipolar (−0.5..+0.5) matching LFO route amounts.
